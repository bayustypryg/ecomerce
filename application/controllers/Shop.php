...
class Shop extends MY_Controller {
    ...
    public function sortby($sort, $page = null)
    {
        ...
        $data['content']    = $this->shop->select(
            ...
            ->join('category')->where('product.is_available', 1)->orderBy('product.price', $sort)->paginate($page)->get();
    ...
    public function category($category, $page = null)
    {
        ...
        $data['content']    = $this->shop->select(
            ...
            ->join('category')->where('product.is_available', 1)->where('category.slug', $category)->paginate($page)->get();
    ...
    public function search($page = null)
    {
        ...
        $data['content']    = $this->shop->select(
            ...
            ->join('category')->like('product.title', $keyword)->orLike('product.description', $keyword)->paginate($page)->get();
